...
# Note that if this is a new project, you need to go to the GCP dashboard and select the Kubernetes Engine
...
gcloud container clusters create ${SPINNAKER_INSTALL_CLUSTER_FULLNAME} \
    --num-nodes=2 --machine-type=n1-standard-2 --cluster-version=latest \
    --enable-legacy-authorization

gcloud container clusters get-credentials ${SPINNAKER_INSTALL_CLUSTER_FULLNAME} \
    --zone ${SPINNAKER_INSTALL_CLUSTER_ZONE} \
